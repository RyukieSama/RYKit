...
- (void)setIndex:(NSInteger)index countAll:(NSInteger)countAll countSelected:(NSInteger)countSelected {
    self.lbIndex.text = [NSString stringWithFormat:@"%ld/%ld",index,countAll];
    if (countSelected == 0) {
        [self.btSelect setTitle:@"确定" forState:UIControlStateNormal];
        self.btSelect.enabled = NO;
    } else {
        [self.btSelect setTitle:[NSString stringWithFormat:@"确定(%ld)",countSelected] forState:UIControlStateNormal];
