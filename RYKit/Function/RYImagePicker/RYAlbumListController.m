...
- (void)setupUI {
    ...
    self.doneButton = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(commitClick)];
    self.doneButton.enabled = NO;
    self.doneButton.tintColor = [UIColor colorWithWhite:0.6 alpha:1];
    self.navigationItem.rightBarButtonItem = self.doneButton;
    
    [self.view addSubview:self.cvImages];
    self.cvImages.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-64-50);
...
- (void)gridCellClickBtSelect:(RYGridCellModel *)cellModel cell:(RYGridCell *)cell{
    //点击之前的index数组
    NSArray *old = [self getItemsNeedReloadWithCellModels:self.cellModels];
    ...
    //点击之后的index数组
    NSArray *new = [self getItemsNeedReloadWithCellModels:self.cellModels];
