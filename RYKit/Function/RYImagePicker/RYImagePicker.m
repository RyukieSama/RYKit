...
- (void)presentImagePickerControllerWithFinishedHandler:(finishedPickImageRY)finished cancelHandler:(canceledPickImageRY)canceled {
    ...
    switch (status) {
        ...
        case PHAuthorizationStatusDenied: {
            //弹窗提示
            UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"提示" message:@"请在iPhone的“设置—隐私—照片”选项中，允许百灵鸟访问你的照片" preferredStyle:UIAlertControllerStyleAlert];
    ...
    //如果在不同的控制器下弹出就清除缓存
    UIViewController *vc = [self getAppTopVieController];
    NSString *currentVCAdd = [NSString stringWithFormat:@"%p",vc];
    
    NSLog(@"当前所在控制器地址 %@",currentVCAdd);
    NSLog(@"上次弹出的控制器地址 %@",self.lastVC);
    
    if ( ![self.lastVC isEqualToString:currentVCAdd] && self.lastVC.length != 0 ) {
        [self resetManager];
    }
    self.lastVC = currentVCAdd;
